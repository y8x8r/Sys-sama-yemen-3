...
Task ID: 12
Agent: Main Agent (Super Z)
Task: إلغاء تكرار العميل في 3 خدمات + إضافة حقلي الوكيل لجوازات بدون حضور

Work Log:
- التعديل 1-3: إلغاء تكرار العملاء حسب رقم الهاتف في القوائم المنسدلة
  * المشكلة: العملاء يظهرون عدة مرات في القائمة المنسدلة إذا كان لديهم نفس رقم الهاتف
  * الحل: إضافة dedupedCustomers في service-page.tsx باستخدام useMemo
    - يُطبق فقط على 3 خدمات: professional_exam, visa_authorization, transit_visa
    - يفحص رقم الهاتف ويُظهر العميل الأول فقط لكل رقم هاتف
    - العملاء بدون رقم هاتف يظهرون جميعاً (لا تكرار للقيم الفارغة)
    - الخدمات الأخرى تحتفظ بالسلوك الأصلي (تعرض جميع العملاء)
  * التحقق: العملاء بدون تكرار في القائمة المنسدلة

- التعديل 4: إضافة حقلي اسم الوكيل ورقم الوكيل لخدمة جوازات بدون حضور
  * تحديث service-configs.ts:passport_without:
    - إضافة agentName (f_agent_name, type: text)
    - إضافة agentNumber (f_agent_number, type: tel)
    - الحقول تظهر في نموذج الإدخال بين cardNumber و requestType
  * التحقق: إنشاء معاملة passport_without بحقول الوكيل نجح — البيانات محفوظة في details

- التعديل 5: إظهار حقلي الوكيل في الإدخال والتفاصيل وإخفاؤها من الفاتورة
  * في الإدخال: الحقول في config.fields تظهر تلقائياً في النموذج ✓
  * في التفاصيل: viewRecord.details يحتوي على agentName و agentNumber ✓
    - نافذة التفاصيل تعرض جميع حقول details مع التسميات الصحيحة
  * في الفاتورة: print/invoice/route.ts (line 58) يتجاوز agentName و agentNumber ✓
    - if (f.name === "agentName" || f.name === "agentNumber") continue;
  * التحقق: طباعة فاتورة passport_without — تأكد أن:
    - اسم الوكيل ورقمه غير موجودين في HTML المطبوع ✓
    - بيانات أخرى (cardNumber, requestType, requestStatus) تظهر بشكل صحيح ✓
    - القيم المترجمة تظهر (جديد، في المكتب) ✓

Stage Summary:
- 3 خدمات (الفحص المهني، تفويض الفيز، تأشيرة العبور): القائمة المنسدلة للعملاء لا تكرر العملاء بنفس رقم الهاتف
- خدمة جوازات بدون حضور: حقلا اسم الوكيل ورقم الوكيل مضافان
- حقول الوكيل تظهر في الإدخال والتفاصيل داخل النظام
- حقول الوكيل مخفية من الفاتورة عند الطباعة
- لا تغيير في أي وظائف أو بيانات أخرى
- ESLint نظيف، خادم التطوير يستجيب بنجاح (HTTP 200)
- اختبار: إنشاء معاملة passport_without مع بيانات الوكيل، طباعة الفاتورة، تأكد أن بيانات الوكيل مخفية

---
Task ID: 13
Agent: Main Agent (Super Z)
Task: إخفاء رقم الهاتف من نموذج الإدخال في 3 خدمات مع إبقائه في الفاتورة

Work Log:
- تشخيص المشكلة:
  * حقل phoneNumber في 3 خدمات كان يحتوي على fromCustomer: "phoneNumber"
  * هذا يجعله يُعرض كقائمة اختيار عميل ثانية (خطأ) بدلاً من حقل هاتف عادي
  * المستخدم يريد إخفاء الحقل من النموذج مع إبقاء رقم الهاتف في الفاتورة

- الحل المطبق:
  1. إضافة خاصية hideInForm?: boolean إلى واجهة FieldDef في service-page.tsx
  2. تحديث phoneNumber في 3 خدمات لإزالة fromCustomer وإضافة hideInForm: true:
     - professional_exam (الفحص المهني)
     - visa_authorization (تفويض الفيز)
     - transit_visa (تأشيرة العبور)
  3. تحديث عرض النموذج: allFields.filter((f) => !f.hideInForm).map(renderField)
     — تخطي الحقول التي تحتوي على hideInForm: true

- كيف يعمل النظام بعد التعديل:
  * النموذج: حقل رقم الهاتف مخفي تماماً — لا يظهر أثناء الإدخال
  * التعبئة التلقائية: onCustomerSelect ما زال يعبئ next.phoneNumber = c.phoneNumber
    — رقم الهاتف يُعبأ من ملف العميل عند اختياره (في الـ state، رغم أنه مخفي)
  * الفاتورة المطبوعة: تعرض رقم الهاتف من سجل العميل في قسم بيانات العميل:
    "رقم العميل: {customerNumber} | الهاتف: {phoneNumber}"

- التحقق:
  * إنشاء معاملة professional_exam: نجح ✓
  * طباعة الفاتورة: رقم الهاتف (777799965) يظهر من سجل العميل ✓
  * الحقول الأخرى (البريد، مهنة الاختبار، كلمة المرور، التواريخ) تظهر في الفاتورة ✓
  * حقل الهاتف لا يظهر في نموذج الإدخال (hideInForm: true) ✓
  * الخدمات الأخرى (جوازات بحضور، جوازات بدون حضور) تحتفظ بحقل الهاتف في النموذج ✓

Stage Summary:
- 3 خدمات (الفحص المهني، تفويض الفيز، تأشيرة العبور): حقل رقم الهاتف مخفي من نموذج الإدخال
- رقم الهاتف يُعبأ تلقائياً من ملف العميل عند اختياره
